...
(function (global, factory) {
  ...
  function () {
    function Html5DashJS(source, tech, options) {
      ...
      setupTextTracks.call(null, this.player, tech, options); // Fix liveui seek issue

      this.tech_.seekable = this.seekable.bind(this);
      this.tech_.setCurrentTime = this.setCurrentTime.bind(this);
      this.tech_.currentTime = this.currentTime.bind(this); // Attach the source with any protection data
    ...
    _proto.seekable = function seekable() {
      ...
      if (this.mediaPlayer_.isDynamic()) {
        return videojs.createTimeRange(sessionStartedTime, sessionStartedTime + dvrWindowLength);
      } // VOD
    ...
    _proto.setCurrentTime = function setCurrentTime(time) {
      var seekable = this.tech_.seekable(); // Livestream
    ...
    _proto.timeDiffFromStart = function timeDiffFromStart(startTime) {
