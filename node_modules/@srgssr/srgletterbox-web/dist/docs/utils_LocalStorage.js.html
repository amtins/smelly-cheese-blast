...
class LocalStorage {
  /**
   * Store a key value in a local storage identified by its name.
   * @param {String} storageName
   * @param {string} key
   * @param {*} value
   */
  static store(storageName, key, value) {
    if (!LocalStorage.storage) {
      return;
    }

    const ls = LocalStorage.getStorageByName(storageName);
    ls[key] = value;

    LocalStorage.storage.setItem(storageName, JSON.stringify(ls));
  }


  /**
   * Get a local storage by its name.
   *
   * @param {String} name
   ...
   * @returns {Object}
   */
  static getStorageByName(name, defaultValue = {}) {
    if (!LocalStorage.storage) {
      return defaultValue;
    }

    const item = LocalStorage.storage.getItem(name);
    let itemValues;
    ...
    try {
      ...
      LocalStorage.storage.setItem(name, JSON.stringify(itemValues));
    }
