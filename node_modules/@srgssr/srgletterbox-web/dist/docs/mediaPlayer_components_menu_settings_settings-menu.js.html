...
<head>
    <meta charset="utf-8">
    <title>JSDoc: Source: mediaPlayer/components/menu/settings/settings-menu.js</title>
...
<div id="main">
    ...
    <h1 class="page-title">Source: mediaPlayer/components/menu/settings/settings-menu.js</h1>
...
class SettingsMenu extends SRGMenu {
  constructor(player, options) {
    ...
    this.continuousPlaybackOptionEnabled = true;
    ...
    player.on(Events.CONTINUOUS_PLAYBACK_INTEGRATION_CHANGE, ({ data: isEnabled }) => {
      this.continuousPlaybackOptionEnabled = isEnabled;
  ...
  updateState() {
    ...
    this.state.quality = this.buildQuality();
    this.state.speed = this.buildSpeed();
  }

  buildContinuousPlayback() {
    if (!this.continuousPlaybackOptionEnabled) {
      return undefined;
    }
    ...
    return {
      ...
      value: () => letterbox.isContinuousPlaybackEnabled(),
      changeEvent: Events.CONTINUOUS_PLAYBACK_CHANGE,
    };
  }

  buildQuality() {
    const { letterbox, mediaComposition } = this.player().options().SRGProviders;

    return {
      label: 'HD',
      type: SRGMenu.PANEL_COMPONENTS_TYPES.TOGGLE,
      value: () => letterbox.isHdMode(),
